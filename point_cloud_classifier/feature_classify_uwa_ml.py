...
def train_model(model_info, feature_path, sample_num, root_path, encode_class):
    # 1. load models
    ...
    patch_path = os.path.join(root_path, 'patches')
    patches = glob.glob(patch_path + '/*')  # abs path

    num_patch_sample = 1000

    for patch_name in patches:
        if 'ply' not in patch_name and 'unknown' not in patch_name and 'back' not in patch_name:
            print('training patch: {}'.format(patch_name))
            # here patch name is class name
            patch_dir = os.path.join(patch_path, patch_name)
            patch_pcd_list = glob.glob(patch_dir + '/*.ply')
            for p in patch_pcd_list:  # each patch in current class
                model, model_down, model_down_fpfh = prepare_dataset(p, voxel_size=voxel_size, trans=init_model_pose)
                patch_feature = np.float32(model_down_fpfh.data.T)[:num_patch_sample]
                label_tmp = np.ones(len(patch_feature)).reshape(-1, 1) * encode_class[os.path.basename(patch_name)]

                feature_buff = np.concatenate((feature_buff, patch_feature), axis=0)  # 叠加特征
                label_buff = np.concatenate((label_buff, label_tmp), axis=0)  # 叠加标签
...
if __name__ == '__main__':
    ...
    model_info = {}
    encode_class = {}  # {model_name: idx}
    decode_class = {}  # {idx: model_name}
    for i, model_path in enumerate(model_path_list):
        ...
        encode_class[model_name] = i
        decode_class[i] = model_name  # {idx: class_name}
    num_models = len(model_path_list)

    if is_get_decode_class:
        # print('decode_class: \n', decode_class)
        decode_class_path = os.path.join(root_path, 'model_lib', 'decode_class.npy')
        encode_class_path = os.path.join(root_path, 'model_lib', 'encode_class.npy')

        np.save(decode_class_path, decode_class)
        np.save(encode_class_path, encode_class)
        print('class decode information saved: {}'.format(decode_class_path))
        print('class encode information saved: {}'.format(encode_class_path))

    if is_train:  # train classifier
        print('training classifier ... ')
        train_model(model_info, object_feature_path, sample_num, root_path, encode_class)
