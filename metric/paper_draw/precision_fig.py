...
p_dict_1 = np.load(precision_path_ori, allow_pickle=True).item()  # .values()
p_dict_2 = np.load(precision_path_my, allow_pickle=True).item()  # .values()
...
bw = 0.3  # 偏移量
my_color = np.array([30, 144, 255]) / 255  # 道奇蓝
ori_color = np.array([255, 140, 0]) / 255   # 橙红
