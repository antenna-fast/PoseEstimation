...
from utils.ransac import ransac_pose
...
if __name__ == '__main__':
    ...
    gt_mat = np.loadtxt('D:/SIA/Dataset/CVLab/' + id_str + SceneFile + 'ground_truth.xf')
    print('gt_mat:\n', gt_mat)

    # 加载bbox
    bbox_path = 'D:/SIA/Dataset/CVLab/3D Models/' + id_str + 'bbox/'
    bbox_np = np.loadtxt(bbox_path + 'obb_robot.txt')

    # 可视化参数
    is_rgb = 1
    inlier_color = np.array([0, 0.3, 1])   # 绿
    outlier_color = np.array([1, 0.3, 0])  # 红
    ...
    # if 1:
        ...
        model_down_np = np.array(source_down.points)  # 模型点
        ...
        source_feature = np.float32(source_fpfh.data.T)
        ...
        for (m, n) in matches:  # m是最小距离  n是次小距离（或者一会加上过滤）
            ...
            if m.distance < flann_ratio * n.distance:  # 什么原理？ 最小的<0.7次小的
                ...
                # np.asarray(source_down.colors)[queryIdx, :] = [0, 1, 0]  # 模型
                # np.asarray(target_down.colors)[trainIdx, :] = [1, 0, 0]  # 场景 对应点着色
    ...
    # np.save(precision_path + 'cvlab_global' + '.npy', precision_dict)
