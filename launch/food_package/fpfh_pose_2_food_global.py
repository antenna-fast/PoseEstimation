...
if __name__ == '__main__':
    ...
    root_path = 'D:/SIA/Dataset/FoodPackage/'
    scene_path = root_path + 'realsense/' + scene + '.ply'

    model_name = '1'
    bbox_path = 'D:/SIA/Dataset/FoodPackage/bbox/'

    # model_idx_list = [0, 3, 4]  # Scene1
    model_idx_list = [1]  # Scene2

    # 精确度保存路径
    # precision_path = 'C:/Users/yaohua-win/PycharmProjects/pythonProject/pose_sys_pnp/measure/precision/'
    ...
    if 1:
        ...
        model_trans_init[:3, 3:] = array([0.2, 0.01, 0.3]).reshape(3, -1)
        source, source_down = load_model(model_path, model_trans_init, voxel_size)

        # 加载对应模型的bbox
        obb_vtx = loadtxt(bbox_path + 'food_' + model_name + '.txt')  # 三维点 可对其进行平移旋转变换
