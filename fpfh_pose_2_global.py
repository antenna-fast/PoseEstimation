...
if __name__ == '__main__':
    ...
    for k in unique_labels:  # 通过匹配这些label，找到mask  k并不能用于类别区分
        ...
        # seg_pcd = np2pcd(seg_pcd_np)  # 前面有了 seg_target
        ...
        # target_feature = float32(seg_target_fpfh_T)
        target_feature = float32(target_fpfh.data.T)  # 场景所有的特征
        ...
        for (m, n) in matches:  # m是最小距离  n是次小距离（或者一会加上过滤）
            # print(m.distance)
            if m.distance < 0.9 * n.distance:  # 什么原理？ 最小的<0.7次小的
        ...
        # scene_target_np = array(target_down.points)  # match改变了索引
        scene_paired = scene_down_np[match_scene_idx]
