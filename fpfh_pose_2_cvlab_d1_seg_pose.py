...
if __name__ == '__main__':
    ...
    # 可视化参数
    is_rgb = 0
    inlier_color = np.array([0, 0.3, 1])
    outlier_color = np.array([1, 0.3, 0])
    ...
    dbscan_class_num = 5  # 前5类

    # RANSAC
    ransac_iter_num = 105  # 评估距离大于此数视为外点
    ...
    # 转换成np
    # model_np = np.array(source_down.points)  # 模型点
    scene_down_np = np.array(target_down.points)  # 场景点
    ...
    for class_member_mask in seg_res_idx:
        ...
        seg_target_np = np.array(seg_target.points)
        ...
        # np.asarray(seg_target.colors) = color_map[class_res]  # 不同类别的颜色  应当加上分类器之后
        np.asarray(target_down.colors)[class_member_mask, :] = color_map[class_res]
        ...
        model_trans_init = np.eye(4)  # 初始化模型位姿，更好地可视化
        model_trans_init[:3, 3:] = np.array([0.5, 0.5, 0]).reshape(3, -1)
        ...
        source_down_np = np.array(source_down.points)  # 模型点
        ...
        # 格式转换 否则flann不支持
        model_feature = np.float32(model_feature)
        target_feature_seg = np.float32(seg_target_fpfh_T)
        ...
        np.asarray(seg_target.colors)[match_scene_idx, :] = [1, 0, 0]  # 分割后的场景  但没有进行可视化
        ...
        res_ransac, confi, inlier, outlier = \
        ...
        draw_registration_result(source, target, res_icp.transformation, is_rgb, 'Ours ICP')  # 配准后
    ...
    # 保存测量值  精确度
    np.save(precision_path + 'syn_my_' + scene + '.npy', precision_dict)
