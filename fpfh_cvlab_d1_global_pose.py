...
if __name__ == '__main__':
    ...
    # for ransac inlier test
    # inlier_threshold = 0.03
    inlier_threshold = 0.08

    # filter matching result which inlier ratio is lower than this
    # inlier_ratio_filter = 0.01
    # inlier_ratio_filter = 0.1
    # inlier_ratio_filter = 0.15
    # inlier_ratio_filter = 0.2
    inlier_ratio_filter = 0.3
    ...
    # data path
    root_path = '/Users/aibee/Downloads/Dataset/3DDataset/6DPose/CVLab/dataset1'
    scene_root_path = os.path.join(root_path, 'dataset1_scenes/3D models/Stanford/Random')
    model_root_path = os.path.join(root_path, 'dataset1-2_models/3D models2/Stanford')  # pure ply file
    scene_file_list = sorted(get_file_list(scene_root_path))
    model_path_list = sorted(get_file_list(model_root_path))
    ...
    pose_ransac_result_path = os.path.join(root_path, 'predict_result', 'global_pose_ransac.npy')
    pose_icp_result_path = os.path.join(root_path, 'predict_result', 'global_pose_icp.npy')
    inlier_ratio_result_path = os.path.join(root_path, 'predict_result', 'global_inlier_ratio.npy')

    # feature matching
    matcher = FeatureMatch(feature_dim=33)

    # logger path
    logger_path = os.path.join(root_path, 'global_pose.log')
    ...
    num_scenes = len(scene_file_list)

    # POSE ESTIMATION START
    s_time = time.time()  # sys start
    for s in range(num_scenes):
        scene_path = scene_file_list[s]
        ...
        if scene_selected not in scene_name:
            continue
        ...
        scene_path = os.path.join(scene_root_path, scene_path)
        ...
        if 1:
            ...
            for i, model_name_rt in enumerate(model_info):  # get key
                ...
                # single model ransac
                res_ransac, loss, inlier_buff, outlier_buff, inlier_ratio = ransac_pose(model_paired, scene_paired,
                                                                                        iter_num=ransac_iter_num,
                ...
                if is_vis:  # visualize icp result
                    draw_registration_result(model_down_rt, scene_down, res_icp,
                                             scene_name + '_RansacGlobal+ICP')  # RANSAC + ICP
                ...
                try:
                    ...
                    inlier_ratio_result.setdefault(model_name_rt, {})
                    inlier_ratio_result[model_name_rt][scene_name] = inlier_ratio
                except:
                    # logger.info('model {} not exist in the current scene '.format(model_name_to_gt[model_name_rt]))
                    logger.info('model {} not exist in the current scene '.format(model_name_rt))
